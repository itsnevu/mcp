...
/* DexScreener uses its OWN chain slugs, which are NOT the GeckoTerminal network slug — so the
   fallback must filter on DexScreener's id, not GT_NETWORK, or it matches nothing and the
   whole fallback is silently dead. Kept as a separate env var for exactly that reason; set
   DEX_CHAIN_ID to whatever DexScreener names Robinhood Chain once it lists it. Empty string
   disables the chain filter (accept every pair the USDG search returns). */
const DEX_CHAIN_ID =
  process.env.DEX_CHAIN_ID !== undefined ? process.env.DEX_CHAIN_ID : GT_NETWORK;
...
function fromDexScreener(json) {
  ...
  return pairs
    .filter((p) => !DEX_CHAIN_ID || p?.chainId === DEX_CHAIN_ID)
    .map((p) => {
