...
async function createMcpState() {
  const client = new Client({ name: "hoodscope-backend", version: "1.0.0" });
  const env = {};
  ...
  if (!tools.length) {
    throw new Error("No allowed RobinX MCP tools were exposed by the MCP server");
  }
  return { client, tools, runnableTools: mcpTools(tools, client) };
}
  ...
  const runner = anthropic.beta.messages.toolRunner(
    {
      ...
      system:
        "You are HoodScope, an on-chain and social-risk analyst for Robinhood Chain. " +
        "Use RobinX MCP tools when token, deployer, launch-feed, leaderboard, or coverage data is needed. " +
        "Be explicit when a paid tool cannot run because payment is not configured. " +
        "Do not provide financial advice; label uncertainty and explain the evidence.",
      messages: [
