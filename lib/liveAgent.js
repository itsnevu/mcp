...
const FIRST_PARTY_TOOLS = [
  ...
  /* The one question a rug check cannot answer from bytecode and current state: if I buy this,
     can I get back out? A honeypot reads identically to an honest token until you try to leave,
     so this is registered as its own tool rather than folded into the rug check — the model
     should be able to reach for it directly when a user asks about exiting a position, and the
     rug check should stay the cheap first look rather than always paying for a swap simulation. */
  {
    schema: {
      type: "function",
      function: {
        name: "robinhood_exit_check",
        description:
          "Prove whether a SELL can actually clear for a token on ROBINHOOD CHAIN (chain id 4663). Use this whenever the user asks if they can sell, exit, or get out of a position, or whether something is a honeypot — a rug check reads the contract, this one simulates the whole sell. Runs read-only: it injects a probe contract into a single eth_call and pays Uniswap V3's swap callback with a real transfer, so the pool's own accounting decides. Nothing is deployed, signed, or spent. Returns EXIT-CLEARS (a sell completed and value came back — at this block, at this size, for a synthetic holder; NOT a promise it will work for the user later), CANNOT-EXIT (the swap reverted — a blocked transfer, a sell tax the pool will not accept, or a pool that will not pay out; a hard signal against buying), or UNKNOWN (no V3 pool, a Uniswap V4 pool id, the node lacks state overrides, or the balance slot was not found). UNKNOWN IS NEVER A PASS — report it as an open question, never as reassurance.",
        parameters: {
          type: "object",
          properties: {
            address: {
    ...
    async run(args) {
      const { simulateExit } = await import("bugglo/simulate");
      return JSON.stringify(await simulateExit(String(args?.address || "")));
    },
  },
];
