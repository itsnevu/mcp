...
// the whole process env (it holds ANTHROPIC_API_KEY, AUTH_SECRET, ROBINX_WALLET_KEY) — pass
...
async function createMcpState() {
  ...
  try {
    ...
    await robinxClient.connect(
      new StdioClientTransport({
        ...
        args: ["robinx-mcp"],
        env: childEnv(["ROBINX_WALLET_KEY", "ROBINX_MAX_USD_PER_CALL", "ROBINX_URL", "HOODSCOPE_URL"]),
      })
    );
  ...
  for (const [name, config] of Object.entries(externalServers)) {
    ...
    try {
      ...
      const tools = filterExternalTools(name, listed.tools || [], config.allowedTools);
      if (tools.length) {
        runnableTools.push(...mcpTools(tools, client));
        mcpClients.push(client);
        console.log(`Loaded MCP server: ${name} with ${tools.length} tools`);
      }
    } catch(e) {
