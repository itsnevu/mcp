...
/* Guard against malformed backend replies before they can crash the renderer. */
export function isValidReply(r) {
  ...
  switch (r.kind) {
    ...
    case "rugcheck":
      return typeof r.address === "string" && typeof r.verdict === "string" && Array.isArray(r.checks);
    case "trending":
...
export function replyToText(reply) {
  ...
  switch (reply.kind) {
    ...
    case "rugcheck":
      return (
        `Rug check ${reply.address}: ${reply.verdict}\n` +
