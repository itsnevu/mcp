...
/* Severity vocabulary of the gate, ordered worst-first to match gate.js's own sort. INFO exists so
   that a reason which is genuinely good news (an OFFICIAL registry match, a sell sim that moved)
   can still be shown without being mistaken for a finding against the token. */
const GATE_SEVERITY = {
  BLOCK: (p) => p.fail("BLOCK  "),
  WARN: (p) => p.warn("WARN   "),
  UNKNOWN: (p) => p.unknown("UNKNOWN"),
  INFO: (p) => p.dim("INFO   "),
};

/**
 * Render a tradeGate() decision.
 *
 * The header states the decision and then immediately qualifies it, because the single most
 * dangerous way to read this output is to see ALLOW and stop. ALLOW never means "safe" — it means
 * no blocker was proven — and UNKNOWN must be acted on exactly like BLOCK. Both facts are printed
 * every time rather than left to a footnote, since the reader who most needs them is the one
 * skimming.
 *
 * @param {object} result  a tradeGate() return value
 * @param {{ paint?: object, width?: number, full?: boolean }} opts
 * @returns {string}
 */
export function renderTradeGate(result, { paint = PLAIN, width = 80, full = false } = {}) {
  if (!result) return "BUGGLO — cannot gate\n\nNo result.";

  const out = [];
  const address = full ? result.address : shortAddress(result.address);
  const decision = result.decision || "UNKNOWN";

  out.push(paint.bold("BUGGLO — trade gate"));
  out.push(paint.dim(`${result.chain || "Robinhood Chain"}${result.chainId ? ` (chain ${result.chainId})` : ""}`));
  out.push(address);
  if (result.side) out.push(paint.dim(`side: ${result.side}`));
  out.push("");
  ...
  out.push(`DECISION  ${headline}`);
  out.push(
    wrap(
      decision === "ALLOW"
        ? "No blocker was proven. This is not a statement that the token is safe."
        : decision === "BLOCK"
          ? "Do not auto-execute. At least one blocker was proven."
          : "Safety could not be proven. Treat this exactly like BLOCK before auto-executing, and escalate to a human.",
      width - 10,
      "          ",
    ),
  );
  ...
  if (!reasons.length) {
    out.push("No reasons recorded.");
  } else {
    for (const reason of reasons) {
      const badge = (GATE_SEVERITY[reason.severity] || GATE_SEVERITY.UNKNOWN)(paint);
      out.push(`  ${badge} ${reason.message}`);
    }
  }

  const unmeasured = result.rug?.unmeasured || [];
  if (unmeasured.length) {
    out.push("");
    out.push(paint.heading("NOT CHECKED — these are not passes"));
    for (const item of unmeasured) {
      const key = typeof item === "string" ? item : item.key;
      out.push(`  ${paint.unknown(UNMEASURED_LABELS[key] || key || "")}`);
    }
  }

  if (result.disclaimer) {
    out.push("");
    out.push(wrap(result.disclaimer, width, ""));
  }
